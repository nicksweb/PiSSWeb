    ...
    <div class="panel-heading">
        <div class="row">
            <div class="col-md-10 text-left">
                <h3 class="panel-title"><?php echo lang('security title security_list_summary'); ?></h3>
            </div>
        </div>  
    </div>
    ...
    <table class="table table-striped table-hover-warning">
        <thead>
            ...
            <tr>
                ...
                <td>
                    <a href="<?php echo current_url(); ?>?sort=LoggedTime&dir=<?php echo (($dir == 'asc' ) ? 'desc' : 'asc'); ?>&limit=<?php echo $limit; ?>&offset=<?php echo $offset; ?><?php echo $filter; ?>"><?php echo lang('security col logged_time'); ?></a>
            ...
            <tr>
                <?php echo form_open("{$this_url}?sort={$sort}&dir={$dir}&limit={$limit}&offset=0{$filter}", array('role'=>'form', 'id'=>"filters")); ?>
                    ...
                    <th<?php echo ((isset($filters['Port'])) ? ' class="has-success"' : ''); ?>>
                        <?php echo form_input(array('name'=>'port', 'id'=>'port', 'class'=>'form-control input-sm', 'placeholder'=>lang('security input zone'), 'value'=>set_value('port', ((isset($filters['Name'])) ? $filters['Name'] : '')))); ?>
                    </th>
                    <th<?php echo ((isset($filters['Status'])) ? ' class="has-success"' : ''); ?>>
                        <?php echo form_input(array('name'=>'status', 'id'=>'status', 'class'=>'form-control input-sm', 'placeholder'=>lang('security input status'), 'value'=>set_value('status', ((isset($filters['db3.Status'])) ? $filters['db3.Status'] : '')))); ?>
                    </th>
                    <th<?php echo ((isset($filters['LoggedTime'])) ? ' class="has-success"' : ''); ?>>
                        <?php echo form_input(array('name'=>'loggedtime', 'id'=>'loggedtime', 'class'=>'form-control input-sm', 'placeholder'=>lang('security input logged_time'), 'value'=>set_value('loggedtime', ((isset($filters['LoggedTime'])) ? $filters['LoggedTime'] : '')))); ?>
                    </th>
                    <th>
                ...
                <?php foreach ($securitys as $security) : ?>
                    <tr>
                        ...
                        <td<?php echo (($sort == 'zone') ? ' class="sorted"' : ''); ?>>
                            <?php echo $security['Port']; ?> - <?php echo $security['Zone']; ?>
                        </td>
                        <td<?php echo (($sort == 'name') ? ' class="sorted"' : ''); ?>>
                            <?php echo ($security['Status']) ? '<span class="inactive">' . lang('admin input active') . '</span>' : '<span class="active">' . lang('admin input inactive') . '</span>'; ?>
                            <?php echo $security['Status']; ?>
                        </td>
                        <td<?php echo (($sort == 'status') ? ' class="sorted"' : ''); ?> colspan="2">
                            <?php echo $security['LoggedTime']; ?>
                        </td>
